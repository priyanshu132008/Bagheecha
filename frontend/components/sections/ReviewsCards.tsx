...
function StarRow({ rating }: { rating: number }) {
  ...
  return (
    <div
      aria-hidden="true"
      className="flex items-center gap-[3px] text-vermillion"
    >
          ...
          <div className="col-span-12 lg:col-span-9 lg:col-start-4">
            ...
            >
              <MaskReveal className="block">
                See what people say about our food.
              </MaskReveal>
            </h2>
          </div>
        </div>
        ...
        >
          <div className="relative h-[28rem] md:h-[34rem]">
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                {/* Decorative opening quote — sits as a watermark
                    behind the text. The body of the quote is the
                    meaning; the glyph is editorial chrome. */}
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 select-none font-display text-[10rem] leading-[0.6] text-espresso/8 md:text-[14rem]"
                >
                  &ldquo;
                </span>
                ...
                {/* Byline — sharp, minimal: name on the left in
                    uppercase tracking, five vermillion stars on the
                    right. No numeric badge, no avatar, no border. */}
                <figcaption className="relative mt-10 flex flex-col items-center gap-4 md:flex-row md:gap-6">
            ...
            {REVIEWS.map((r, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => goTo(i)}
                  className={cn(
                    "h-1.5 rounded-full transition-[width,background-color] duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]",
                    active ? "w-8 bg-vermillion" : "w-1.5 bg-espresso/15",
                  )}
                />
              );
            })}
